...
export function buildCleanPlanningHtml({ year, month, auxiliaries = [], schedule = {} }) {
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>Planning-AVD - ${MONTHS[month]} ${year}</title><style>
    @page{size:A4 landscape;margin:8mm}
    *{box-sizing:border-box}body{margin:0;padding:18px;font-family:Inter,Arial,sans-serif;color:#26333a;background:#fffefa}
    .sheet{min-height:calc(100vh - 36px);padding:18px;border:1px solid #d8e3e6;border-radius:18px;background:linear-gradient(135deg,#ffffff,#fbfbf3 55%,#fff7fb);box-shadow:0 16px 42px rgba(84,111,124,.12)}
    ...
    .badge{padding:8px 12px;border-radius:999px;background:#e7f7fa;color:#17645e;font-weight:900}
    ...
    footer{margin-top:12px;color:#6e7c84;font-size:10px;font-weight:700;text-align:right}
    @media print{body{padding:0;background:#fff}.sheet{box-shadow:none;border-radius:0;border:0;min-height:auto}footer{display:none}}
  </style></head><body><section class="sheet">
    <header><div><h1>Planning-AVD</h1><p>${MONTHS[month]} ${year} · vue propre equipe</p></div><div class="badge">${active.length} auxiliaire(s)</div></header>

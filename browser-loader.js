(async () => {
  ...
  const googleConfigView = `function ConfigView() {
    ...
    return <div className="su" style={{display:"grid",gap:10}}>
      ...
      <div style={{...card,padding:12}}><b style={{color:"#31556F"}}>Emails auxiliaires</b><p style={{fontSize:12,color:"#667F94",margin:"6px 0"}}>Ces adresses restent dans la sauvegarde personnelle du compte admin connecte.</p>{connected ? TIDS.map(p=><input key={p} type="email" value={auxEmails[p]||""} onChange={e=>setAuxEmails(x=>({...x,[p]:e.target.value.trim()}))} placeholder={"Email "+pName(names,p,priv)} style={{width:"100%",boxSizing:"border-box",marginTop:7,padding:10,borderRadius:12,border:"1px solid #D6E7F5"}} />) : <div style={{...btn(false),width:"100%",textAlign:"left"}}>Connexion Google requise</div>}</div>
